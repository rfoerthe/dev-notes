...
const ProfileThemeSync: React.FC = () => {
  ...
  const profileThemeMode = userProfile?.themeMode;
  const profileThemeAccent = userProfile?.themeAccent;
  const profileUid = userProfile?.uid;

  useEffect(() => {
    if (loading || !profileUid) {
      return;
    }

    setThemeMode(profileThemeMode || 'system');
    setThemeAccent(profileThemeAccent || 'purple');
  }, [loading, profileThemeAccent, profileThemeMode, profileUid, setThemeAccent, setThemeMode]);
